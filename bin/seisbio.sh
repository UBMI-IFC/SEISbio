...
if [[ "$(pwd)" != "$SEISBIO_HOME" ]]; then
    ...
    if [[ -d "$SCRIPT_DIR/../tutorial-seisbio" ]] && (sudo -i -u seisbio bash -c "conda env list 2>/dev/null | grep -qw tutorial-env" || [[ -d "$SEISBIO_HOME/miniforge/envs/tutorial-env" ]] || [[ -d "$SEISBIO_HOME/miniconda/envs/tutorial-env" ]]); then
        if [[ ! -d "$SEISBIO_HOME/tutorial-seisbio" ]]; then
            ...
            sudo chown -R seisbio:seisbio "$SEISBIO_HOME/tutorial-seisbio"
        else
    ...
    echo "[INFO] Opening session as seisbio..."
